...
int main() {
    ...
    std::sort(emails.begin(), emails.end(), [](const std::string& a, const std::string& b) {
        // '@'より前の部分だけを比較
        std::string id_a = a.substr(0, a.find('@'));
        std::string id_b = b.substr(0, b.find('@'));
        return id_a < id_b;
        });
...
// 
// のような配列がある。こちらを学糖番号順に並べて表示したい
// ↑の配列についてはvector<string>で直言すること
// ↑は自分でVectorなど手動で初期化して良い